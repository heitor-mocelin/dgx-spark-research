...
- **Source-build vLLM for `sm_121` — we checked the binaries; it's NOT worth it.** `cuobjdump` on the prebuilt `.so` files shows vLLM and FlashInfer already ship **Blackwell SASS (`sm_120`, `sm_100`)** with **no PTX and no generic fallback** — and `sm_120` (12.0) cubins run natively on GB10 (`sm_121`, 12.1) via forward minor-version compatibility. That's why GLM/Qwen ran at expected speed. So a from-source `arch=121` build buys ~nothing. The "Marlin fallback" we saw was a vLLM *capability-check code path* (steered to FlashInfer by env vars), **not a missing binary**. **DeepSeek-V4's real blocker is a specific missing FlashInfer `trtllm` NVFP4-MoE kernel for Blackwell** — it needs an *updated FlashInfer*, not a vLLM recompile. Real future work: a newer FlashInfer/vLLM that ships the Blackwell NVFP4-MoE `trtllm` kernel. *(Correction: an earlier draft claimed the prebuilt was sm_121-crippled and that compiling would unblock DeepSeek-V4 — the binary inspection refutes both.)*

#!/usr/bin/env bash
# launch.sh — serve Qwen3-32B (NVFP4, dense) on the GX10 via vLLM. The exact recipe used in the
# test matrix. Weights: nvidia/Qwen3-32B-NVFP4 (~18 GB). Override any knob via env.
set -euo pipefail
DOCKER="${DOCKER:-docker}"                 # in the docker group → no sudo
IMAGE="${IMAGE:-vllm/vllm-openai:nightly}"
CONTAINER="${CONTAINER:-qwen3-32b}"
MODEL_DIR="${MODEL_DIR:-/home/user/models/qwen3-32b-nvfp4}"   # huggingface-cli download nvidia/Qwen3-32B-NVFP4
...
echo "[qwen3-32b] waiting for /health on :$PORT (cold start ~5-6 min)…"
...
echo "[qwen3-32b] healthy → http://localhost:${PORT}/v1 (served as $SERVED_NAME)"
